...
const checkCar = car.includes('Lada');
...
const newComments = commentsMedia.filter(comment => comment.email.includes('.com'));
...
const users = commentsMedia.map(user => {
  if (user.id <= 5) {
    return {
      ...user,
      postId: 2
...
const userSummary = commentsMedia.map(user => ({
  id: user.id,
  name: user.name
}));
...
const updatedMessages = commentsMedia.map(message => ({
  ...message,
...
const emails = commentsMedia.reduce((acc, item) => {
  acc.push(item.email);
  return acc;
}, []);

const emailArray = commentsMedia.map(item => item.email);
